...
- Worker-backed FCS import path parses dropped files off the UI thread when served locally, with streamed byte-level progress, event-level parse progress, cancellation, direct parser fallback for constrained contexts, full event parsing for small local files, capped previews for mid-sized files, and metadata-only preflight for oversized files.
...
- Shared axis transform module for linear, log, logicle / biexponential, and arcsinh transforms, with inspector controls for log floor, transform width, arcsinh cofactor, auto-tuned defaults from the active parameter range/event preview, scale-aware axis ticks, and visible per-axis scale validation cues.
...
- Optional clinical mode remains off by default and adds local authentication/role proof, user registry, locked/finalized versioning, electronic signature records, immutable-style hashed audit preview, retention/access settings, and compliance export proof without making validation claims.
...
- 21 CFR Part 11 validation, lab SOP/legal review, production identity provider integration, validated e-signatures, retention enforcement, and clinical compliance.
